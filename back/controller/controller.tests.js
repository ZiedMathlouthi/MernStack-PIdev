...
exports.getAllTestsObject = async (req, res) => {
    try{
        await testModel.find({}).then(
            (result) => {
                res.status(200).send(result);
            }
        ).catch(
            (error) => {
                res.status(200).json({ message: `Error while getting all Tests. Error:\n${error}` });
            }
        );
    }catch(error){
        res.status(200).json({ message: `error while finding from testModel. Error:\n${error}` });
    }
}

exports.getAllTestsData = async (req, res) => {    
    try {
        const testsObjectArrayPromises = await testModel.find({}).then(
            (result) => { return result }
        );
        const testsObjectArray = await Promise.all(testsObjectArrayPromises);
        const questionsIdsArray = testsObjectArray.map((test) => {return test.listOfQuestions});
        const questionDataPromises = questionsIdsArray.map(async(question) => {
            const sb = question.map(async (id) => {
                const Promises = await questionModel.findById(id)
                .then( (res) => {return res} )
        ...
        res.status(515).json({ message: "ERRORRR !!!!!!"+ error })
    }
}
...
exports.getTestObjectById = async (req, res) => {
    id = req.params.id;
    try {
        const testObject = await testModel.findById(id).then(
            (result) => { return result; }
        ).catch(
            (error) => res.status(400).json({ message:"Error while finding a test"+error })
        );
        res.status(200).send(testObject)
    } catch (error) {
        res.status(400).json({ message:"Error getting a test by id" });
    }
}
...
exports.getTestDataById = async (req, res) => {
    id = req.params.id;
    try {
        const testObjectById = await testModel.findById(id).then(
            (result) => {return result}
        ).catch(
            (error) => {res.status(400).json({ message:"Error while getting test by Id" });}
        );
        const questionsIdsArray = testObjectById.listOfQuestions;
        const sb = questionsIdsArray.map(async(questionId) => {
            const Promises = await questionModel.findById(questionId)
            .then( (res) => {return res})
        ...
        if(!user){
            ...
            if(test.testOwner.toString() === user.id){
                ...
                await testModel.findByIdAndUpdate(id,updatedTestContent).then(
                    (RES)=> res.status(200).send("success")
